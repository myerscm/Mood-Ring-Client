...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://pure-journey-24735.herokuapp.com',
...
var songEvents = __webpack_require__(11);
...
var onGetSongs = function onGetSongs(event) {
  ...
  api.getMySongs().then(ui.onMySongsSuccess);
};

var onCreateSong = function onCreateSong(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.createSong(data).then(ui.onCreateSongSuccess).then(function () {
    return onGetSongs(event);
  });
};

var onDeleteSong = function onDeleteSong(event) {
  event.preventDefault();
  var songId = getFormFields(event.target);
  api.destroySong(songId.song.id).then(ui.onDeleteSongSuccess).then(function () {
    return onGetSongs(event);
  }).catch(ui.onError);
};
...
var onUpdateSong = function onUpdateSong(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.updateSong(data).then(ui.onUpdateSongSuccess).then(function () {
    return onGetSongs(event);
  }).catch(ui.onError);
};

var hideSongs = function hideSongs() {
  event.preventDefault();
  $('#songlist').addClass('hidden');
  $('#hidesongs').addClass('hidden');
};

var showCreateForms = function showCreateForms() {
  event.preventDefault();
  $('#createsongs').removeClass('hidden');
};

var showRemoveForms = function showRemoveForms() {
  event.preventDefault();
  $('#removesongs').removeClass('hidden');
};

var showUpdateForms = function showUpdateForms() {
  event.preventDefault();
  $('#update').removeClass('hidden');
};

var hideCreate = function hideCreate() {
  event.preventDefault();
  $('#createsongs').addClass('hidden');
};

var hideDestroy = function hideDestroy() {
  event.preventDefault();
  $('#removesongs').addClass('hidden');
};
...
var getMySongs = function getMySongs(data) {
  return $.ajax({
...
var destroySong = function destroySong(songId) {
  return $.ajax({
...
var onMySongsSuccess = function onMySongsSuccess(data) {
...
var onCreateSongSuccess = function onCreateSongSuccess(data) {
...
var onDeleteSongSuccess = function onDeleteSongSuccess(data) {
...
var onUpdateSongSuccess = function onUpdateSongSuccess(data) {
...
module.exports = {
  onMySongsSuccess: onMySongsSuccess,
